...
- **In progress on `develop` (unreleased):** the continuation-file change set — the hand-off is written at EVERY sprint close (unconditional, whatever `Autonomy:` and `Chaining:` say) and regenerated as the work advances so it is never stale. Four reading surfaces plus the lock block (D-012) — and the arriving session now READS it first, before the state files, so a bare "continue" restarts the work with no recap (D-013).

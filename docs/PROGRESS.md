...
- Keel baseline: v5.8.0 — this repository authors the version it is on, so the baseline always equals the version being written.
- Website intent: no
...
- Branches: integration branch `develop` (created from `main` 2026-07-30 and published) / no open work branch / nothing awaiting `main` — v5.8.0 merged and tagged on the user's explicit instruction
...
## Current position
- Phase: maintenance — **v5.8.0 RELEASED** (2026-07-31): forge issues stop waiting for the next sprint to END
- Step/sprint: v5.8.0 change set — the sprint KICKOFF sweeps the forge before it plans anything, gated on `docs/issues.md`'s new `Last inbound sweep:` line against the card's new `Issue sweep interval:` (default 24h), so the gap between two sprints stops hiding fresh reports and `awaiting reporter` replies. Two `MANIFEST.md` table defects fixed. Lock block UNCHANGED — stamp-only refresh. See D-015, D-016
...
- **Ready for `main`:** nothing. v5.8.0 shipped; `main` and `develop` are level at the release commit.
...
Last updated: 2026-07-31 — maintenance, v5.8.0 released

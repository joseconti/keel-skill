...
- Keel baseline: v5.8.1 — this repository authors the version it is on, so the baseline always equals the version being written.
- Website intent: no
...
- Branches: integration branch `develop` (created from `main` 2026-07-30 and published) / no open work branch / nothing awaiting `main` — v5.8.1 merged and tagged on the user's explicit instruction
...
## Current position
- Phase: maintenance — **v5.8.1 RELEASED** (2026-07-31): the chained chat stops deadlocking on the chat that launched it
- Step/sprint: v5.8.1 change set — the single lane is released at the CLOSE-OUT (`scripts/keel-handoff-verify --release`), never "on exit", a moment a session never reaches; `scripts/keel-continue` records its PID in the launch receipt and releases before firing; an arriving session takes the BATON from a live holder that is this hand-off's recorded launcher; a busy-lane STOP names the remedy. Reported from a real `Chaining: start` project that deadlocked at link one. Lock block UNCHANGED — stamp-only refresh. See D-017, D-018
...
- **Ready for `main`:** nothing. v5.8.1 shipped; `main` and `develop` are level at the release commit.
...
Last updated: 2026-07-31 — maintenance, v5.8.1 released

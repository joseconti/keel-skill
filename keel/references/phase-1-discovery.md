...
Then create `docs/PROGRESS.md`, `docs/decisions.md`, `docs/lessons-learned.md` and `docs/sessions.md` from the templates in that reference, filling the project card with what is known so far (the rest is filled as this phase decides it). In the same step generate `scripts/keel-time` ("Session time" in that reference), add `docs/.keel/clock.jsonl` to `.gitignore`, and run `scripts/keel-time start`, so this very session is timed from its first slice (SKILL.md, "Session time — measured, never remembered"). From this moment on, every decision goes to `decisions.md` and every position change goes to `PROGRESS.md` — at the moment it happens, not at phase end. Stamp the project card's `Keel baseline:` line with the running Keel version — the post-update reconciliation (`references/project-state.md`) relies on it.
...
- **Offer the native assistant config package** (optional — per `references/assistant-config.md`): ask which assistants will work on this repo, then offer their native config — path-scoped rules and reviewer/verifier subagents generated at Phase 2 close from the recorded conventions and security profile, permission allow-lists confirmed with the user, the confidential-data pre-commit gate, and MCP registration when the plan defines development MCP servers — one container per accepted tool, each piece materialized when its source exists, never now. Ask it in the same batch as the embed question; record `Assistant config: [none / rules / rules+agents / full] (tools: ...)` in the project card. **In that same batch, where the package is accepted and the forge has CI, ask WHEN the CI workflow runs** and record `CI runs on:` — `main` (the default: push to `main`, version tags, and pull requests targeting `main`), `main+develop`, or `all-branches`. Offer the default with its reason and its cost in one breath: Keel commits to `develop` many times per sprint and already drove the full suite locally seconds before each one, so a workflow on every push re-runs what just passed and buries the result nobody reads — while a break that only CI's environment surfaces (another OS, a clean checkout, a dependency that exists only on this machine) then waits until the merge. **Where the forge is GitHub and the repository is private, name the concrete stake too, not only the noise:** Actions minutes on the Free/Team tier are billed PER ACCOUNT, shared across every private repository, and exhausting them does not fail loudly — a run is simply never scheduled, with nothing to read a failure from (`references/assistant-config.md`, "WHEN it runs is the user's decision" — measured on this skill's own repository). Check visibility with the forge's own tool where one is available (e.g. `gh repo view --json isPrivate`) rather than asking the user to remember it. It is never inferred, least of all from `Autonomy:`: a fully automatic project can still want `develop` green for a human collaborator. Mention once that every tool has personal, never-committed files (`CLAUDE.local.md`, `.claude/settings.local.json`, Codex's `AGENTS.override.md`) — Keel never creates them but always gitignores them.
...
- Compliance/data concerns (PII, payments, auth) — flag these now so the security profile is applied early. The same facts later decide the card's `Security audit:` line at Phase 2 §4c (`references/security-audit.md`).
...
Ask once, alongside the other opening decisions, and record the answer as `Chaining:` on the project card. It is NOT a card field to fill in silently: two of the four values change how the user works, and they must know that before choosing, not after. **Part of the question is factual rather than a preference — is continuation handled by something OUTSIDE this project?** An external supervisor, a scheduler, a CI runner or a person all answer yes, and the answer is `supervised` with that named on the card; a project under external supervision recorded as `off` has lost the reason, and Keel's own chaining would fight the supervisor by opening a session it cannot reach (`references/project-state.md`).
...
`off`, `supervised` and `prefill` are always available. **`start` is gated**: it is not offered until the project has the single-lane lock (`references/project-state.md`), and it is verified on macOS only — offer `prefill` as the maximum where either condition fails, and say which one. Whatever the answer — `supervised` included — the hand-off file is written and the prompt shown at every session end; chaining only decides whether a window also opens. Full contract in `references/project-state.md`.
...
- `claude` on PATH: [yes / no + install offered and the user's answer / n/a — Chaining: off or supervised] (no → start is not offered; prefill is the maximum)
...
- Chaining: [off / supervised — <what supervises it> / prefill / start — default follows `Autonomy:`: automatic → the maximum tier the gates allow, otherwise off. `supervised` = continuation is handled outside this project (external supervisor, scheduler, CI runner, person), so nothing is opened here and `scripts/keel-continue` is never fired; it counts as `off` at every chaining gate. Asked once here with its warning shown; start is gated on the single-lane lock and macOS-verified only, so offer prefill as the maximum where either fails]
- Chaining model: [the model every chained chat launches with, e.g. `opus` — asked in the same breath as `Chaining:` whenever the answer is `prefill` or `start`, proposing the model in use in this session. Passed as `claude --model <value>` at every link, because a new process inherits nothing from the one that launched it. `n/a` when `Chaining: off` or `Chaining: supervised`]
...
- The chaining question was asked once, WITH its warning shown (that `start` moves development to CLI sessions rather than the editor, and that it removes the only person supervising) AND with the factual half asked plainly (is continuation handled by something OUTSIDE this project?), and `Chaining:` is on the project card — carrying WHAT supervises it where the answer is `supervised`, and together with `Chaining model:` whenever the answer is `prefill` or `start`, since an unstated model means the chain silently resolves one from settings rather than keeping the one in use. If the answer was `start`, the single-lane lock exists and the platform is verified; otherwise `prefill` was offered instead and the reason recorded.

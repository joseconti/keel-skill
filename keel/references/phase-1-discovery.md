...
- Where `Chaining:` is `prefill` or `start`, `command -v claude` was run, the install was offered if it was missing, and the answer is recorded. Absent and declined → the card says at most `prefill`, naming the requirement that failed.

...
| Tool rows (`scripts/keel-tools/<tool>.sh`, one per accepted tool) | The tool registry in `references/project-state.md` + what `--smoke` verified on this machine | Phase 5 scaffold | A tool is accepted or dropped, a vendor changes its invocation, or a row is promoted to `VERIFIED` |
...
| Tool row — the chaining action, the CLI name, whether the card's model may be passed, whether the stop hook's schema is this tool's | `scripts/keel-tools/claude.sh` | `scripts/keel-tools/codex.sh` | `scripts/keel-tools/copilot.sh` | `scripts/keel-tools/cursor.sh` | `scripts/keel-tools/gemini.sh` | `scripts/keel-tools/windsurf.sh` |
| Stop hook (turn-end, invoked by the ASSISTANT — not `.githooks/pre-commit` below) | `.claude/settings.json` → `hooks.Stop` (`scripts/keel-stop-hook`, native contract) — VERIFIED | — (contract undocumented/unconfirmed) | — (contract undocumented/unconfirmed) | — (contract undocumented/unconfirmed) | — (contract undocumented/unconfirmed) | — (contract undocumented/unconfirmed) |

Notes the generators must respect:

- **The tool row is the one piece whose container is Keel's rather than the vendor's**, and that is deliberate. Every other row of the matrix lands in a directory the tool itself reads, so its shape is the tool's to dictate; the row exists precisely because the SHARED scripts need one place to look up what differs between assistants, so its shape is fixed by `references/project-state.md` ("The registry is DATA") and identical everywhere. It is also the only piece a session may write for its OWN tool and no other: repairing another assistant's row from outside it is done blind, against a tool this session cannot run, and it is the measured failure the container was built to end.
...
- **A hook's TRIGGER existing in a tool is not evidence its CONTRACT matches another tool's.** `scripts/keel-stop-hook`'s output is Claude Code's own `Stop`-hook JSON schema (`hookSpecificOutput`, `decision: "block"`), and it is NEVER rewritten to serve a different tool — that would break the one integration already proven. It is registered ONLY where that exact schema is CONFIRMED against the target tool's own current documentation — today, Claude Code alone, and **the generator reads that from the row's `KEEL_TOOL_STOP_HOOK` field rather than re-deciding it from this paragraph**; every other cell of that row is `—` for the same reason a permission-allow-list cell is `—`: no committable mechanism exists yet, never a guessed one shipped as if it were verified. This is the OPPOSITE case from `.githooks/pre-commit` below, which stays genuinely portable because GIT invokes it, not the assistant, so its output only ever has to satisfy git. A `Stop` hook is invoked by the ASSISTANT ITSELF, so its accepted output is that assistant's own, sometimes undocumented, private contract — measured: Codex accepted the identical script Claude Code runs, from its OWN `.codex/hooks.json`, and rejected the output with "invalid stop hook JSON output," ending the very turn the hook exists to keep open, on a project running both tools against the same repository. Full account in `references/project-state.md` ("`scripts/keel-stop-hook`" — "Which tools it is registered in").
...
**The suite it runs follows the trigger (UNBREAKABLE).** A run on `main`, on a version tag or on a PR targeting `main` is a release boundary and runs the ENTIRE suite. A run the wider values add — a push to `develop`, or to any branch — runs `scripts/keel-affected-tests --run --base <the push's before-sha>` instead, the same selection `.githooks/pre-push` ran locally (`references/test-automation.md`, "Which tests run when"); the whole suite on every integration push is exactly the cost that rule removes, moved to the forge's minutes.
...
**Where the forge is GitHub and the repository is PRIVATE, the metered minutes are their own reason, not folded silently into "less noise."** GitHub Actions' included minutes on the Free and Team tiers are billed PER ACCOUNT (or per organisation), pooled across EVERY private repository — never per repository — while a public repository runs unmetered on the same GitHub-hosted runners regardless of plan. A workflow keyed to every push on one private repo is not spending only that repo's budget; it is a tax on every OTHER private repo sharing the same account, including ones that already chose `main`. **Measured, on this skill's own repository:** a completely ordinary, confirmed-on-the-remote tag push produced NO workflow run whatsoever — not a failed one, not a queued one, nothing to read a failure from — because the account's included minutes were exhausted. The gap surfaced only because the expected GitHub Release never appeared, and it took a genuine diagnostic pass (the tag on the remote, the workflow's enabled state, the raw Actions API) to tell that apart from every other reason a tag push can fail to produce a release. Check visibility with the forge's own tool where one is available (e.g. `gh repo view --json isPrivate`) rather than asking the user to remember it, and name this risk in the same breath as the signal-to-noise argument whenever the repository is private — it is what makes the conservative default protection for every other project in the account, not merely tidiness for this one.

...
**Complete the Claude config package (if accepted at step 0a — per `references/claude-config.md`):** propose `.claude/settings.json` with a minimal allow-list built ONLY from the technical plan's verified tooling and playground commands, and write it on the user's confirmation; install the confidential-data pre-commit gate (`.githooks/pre-commit` + `git config core.hooksPath .githooks`) and VERIFY it blocks a synthetic secret before slice 1; create `.mcp.json` only if the technical plan defines development MCP servers (environment expansion, never a literal secret). Package or no package, the scaffold's `.gitignore` ALWAYS includes `CLAUDE.local.md`, `.claude/settings.local.json`, and `.keel-update-check` (the machine-local update-check throttle stamp).
...
- If the Claude config package was accepted: `settings.json` confirmed with the user, the pre-commit gate installed and verified against a synthetic secret, `.mcp.json` present only if the plan defines dev MCP servers (per `references/claude-config.md`).

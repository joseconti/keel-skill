...
Before tagging:
- **Executed checks, not log reading.** "Phase 5 test points all pass" is verified by RE-RUNNING now, on the release candidate — never by reading `docs/05-test-points.md`: run the ENTIRE automated suite against the exact distributable being tagged and record command + result in `docs/07-release.md`, with its scope line `scope: full — M of M tests` — **this gate is the one place the entire suite is mandatory** (every push before it ran only the affected selection, per `references/test-automation.md` "Which tests run when"), so a count below the suite's total, or a `scope: affected` line here, fails the gate; run `scripts/keel-verify` on the candidate — clean, output recorded. Phase 6 docs complete.
...
- **Active security audit, per the card's `Security audit:` line** (`references/security-audit.md`, "The Phase 7 gate"). `required`: a run covers the candidate (a full run at the candidate, or a full run from this cycle plus a scoped run over the diff since it); no confirmed finding is open (each one is fixed with its verifier re-run against the candidate, or accepted by a D-entry into "Not defended"); every `needs_validation` item is resolved or acknowledged by the user; and the run's row exists in `docs/security-audit.md`. The run's own files stay gitignored, and `docs/07-release.md` records the counts and the run's directory, never the open findings. `declined`: one line naming the D-entry. `optional`: offer the audit in one line and continue.
...
- **The declared end-to-end suite (gate — only where the project card carries an `E2E:` line).** Absent line, absent gate: nothing runs, nothing is written, nothing is said, which is the state of every project that has not opted in. Where the line exists, the gate passes only on `docs/.keel/e2e-status.json` carrying `result: "pass"` with `commit` equal to `HEAD`. **A missing file, a stale file (a `commit` that is not `HEAD`), an `error` and a `fail` all block, and the message says which of the four it was** — a stale result and a failed suite are different facts, and blurring them sends whoever reads it to debug the wrong thing. A waiver is a `docs/decisions.md` entry written BEFORE the release proceeds, naming what was waived and why; no flag skips this gate without leaving a record. This gate does not replace the full driven-suite re-run above and never softens it — what is optional is the PUBLICATION of an end-to-end result, never the verification. Full contract in `references/project-state.md` ("The end-to-end verification contract").
...
- Card `Security audit: required`: an active audit covers the candidate with no confirmed finding open and every `needs_validation` item resolved or acknowledged, and its counts-only row is in `docs/security-audit.md` — or the line is `declined` with its D-entry (`references/security-audit.md`).

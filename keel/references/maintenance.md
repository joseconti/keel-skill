...
- **Sweep both directions before triaging anything.** New issues since the last session, AND new comments on issues that already exist — the second is what tells you whether an issue sitting in `awaiting reporter` has been answered, and in maintenance that gap is measured in weeks. Read them by status per `references/project-state.md`, record what came back in the entry's `Inbound:` field, and only then decide what gets worked.

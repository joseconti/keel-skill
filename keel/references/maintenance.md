...
- **The clock, before any work.** `scripts/keel-time start` — the time, what is left, and what this session plans, with hours — and `scripts/keel-time end` with its report when the session stops (SKILL.md, "Session time — measured, never remembered"). A hotfix is timed like anything else: the deviation of urgent work is exactly the number a client asks about afterwards.
- **The plan, before any work.** Every maintenance item — a triaged report, a hotfix, an audit command's finding, a CVE response, a "Tested up to" bump, a feature — is a slice in `docs/sprints/` with its hours BEFORE its first change, marked `done` with its `actual_hours` in the commit that finishes it, with `docs/.keel/plan.json` regenerated in that commit (SKILL.md, "Sprints are the ledger of all work"). A released project reopened with no plan gets one now, as a maintenance sprint built from what is on the table, and it is shown to the user in the same message. Urgency does not exempt a hotfix: adding its slice is one line and takes seconds. Only `Sprints: off` on the card, with its D-entry, lifts this.
...
## The test-scope rule — the change at every push, the whole suite at every release

EVERY maintenance change, however small, runs the selection `scripts/keel-affected-tests` derives from its diff before each push (`references/test-automation.md`, "Which tests run when") — and the ENTIRE automated suite before the release that ships it, one-line fixes and hotfixes included: at a release the blast radius of a change is what the suite exists to measure, not what the author assumes. Between releases, the selection's own widening rules are what stand in for that assumption. And every fixed bug carries its regression test, written and failing BEFORE the fix (a hotfix without one is not done, per the hotfix path — the rule holds on every project whatever its `Test-first policy:` line says) — the suite only protects against what it contains.
...
- The change was a slice in `docs/sprints/` with its hours before its first edit, and the commit that finished it set `done`, wrote `actual_hours` and regenerated `docs/.keel/plan.json` — or the card says `Sprints: off` with its D-entry.
...
- The fix STARTED from its reproduction test — written before the fix and seen to fail on the bug — and that test stays as the regression test, driven where the bug is user-visible (the only exception is the recorded one: a `docs/decisions.md` entry stating the fix shipped without it and what that costs); the affected-test selection was green at every push and the FULL automated suite is green on the release candidate; `scripts/keel-verify` clean; real-environment verification passed on the rebuilt distributable with the flow driven by the assistant, and anything undrivable carrying its tag and its steps.

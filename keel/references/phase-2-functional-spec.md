...
- Test selection (the source of scripts/keel-affected-tests — per references/test-automation.md "Which tests run when"): the impact tool this stack uses (or the path convention from a source file to its tests plus how reverse dependencies are found), the always-run smoke set if any, and this project's concrete paths for the closed widening list (its lockfiles, test bootstrap, shared fixtures, build and environment configuration, migrations). Every push runs that selection; the entire suite runs at the Phase 7 gate

...
6. **It is checked, not trusted.** `scripts/keel-verify` gains three rows: every tool named on the card's `Assistant config:` tools list has a row file; every row file declares every field above; and **no shared `scripts/keel-*` matches a registry tool name outside a comment.** The third is the one that catches the regression this section exists to end, because it is the only one that fails when somebody "just adds an `if`".

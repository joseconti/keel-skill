...
4. **Assets are real, indexed, and delivered ready for the build to use directly — never something the build must transform.** Exported files live in `artifacts/assets/`. `SPEC/assets-index.md` maps each file to where it is used, its intrinsic dimensions, and format. No "an icon goes here" without the actual icon. On top of that, two hard delivery rules:
   ...
   - **Fonts are assets under the same rule.** The handoff ships the actual self-hosted font files — only the weights/styles actually used, in the formats the target platform needs (for the web, `woff2` first) — at their final paths under `artifacts/assets/fonts/`, with the `@font-face` rules (or the platform equivalent) in the delivered styles already written against those exact paths. When licensing or availability prevents shipping a font file, the handoff includes instead, per font: the exact download source (the foundry or official page), the license note, the exact files/weights/styles to obtain, and the exact target path inside the delivered structure — written so the user can drop the files in and everything works, and reuse them in future projects, without editing anything. The Phase 4 guided loop (Step 6) walks the user through that acquisition one font at a time. A font that is merely named — no files and no acquisition block — is an incomplete handoff.
...
7. **External setup is fully extracted, never implicit.** If any part of the project requires a human to configure external software (Unity, a hosting panel, an OAuth provider console, a SaaS settings page, DNS, etc.), every exact configuration value MUST be written into `SPEC/external-setup.md` — field names, exact values, toggles, order. It is a contract violation to leave a needed config value implicit inside an artifact file and not surface it in the SPEC. Downstream this is consumed by an interactive, step-by-step human walkthrough: a value that is not in the SPEC cannot be guided safely and becomes a Design Request. Where a value is the user's to decide, Design must ask the user, not invent it.
   ...
   **Secrets are the one carve-out.** A value that is a secret — an API key, a client secret, a payment-gateway merchant key, a password, a token — is NEVER written into `SPEC/external-setup.md` or anywhere else in the handoff. The file records a descriptive placeholder instead (e.g. `<client secret — the user enters it during the Phase 4 guided walkthrough; never written here>`); the real value is exchanged only inside that interactive loop, at the step that needs it. Non-secret values (account identifiers that grant nothing by themselves, domains, plan names) are recorded normally. This is what keeps the handoff committable under the cross-cutting confidential-data rule (SKILL.md).
...
- `SPEC/design-tokens.md` states its origin and maps the canonical token set onto every target surface named in the brief (per rule 3) — a single-surface token file for a multi-surface brief is incomplete.

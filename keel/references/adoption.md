...
**Create the plan in the same step, before the adoption's own work begins** (SKILL.md, "Sprints are the ledger of all work"): write `Sprints: on` on the card, and open `docs/sprints/sprint-0.md` — the adoption itself — with one slice per remaining adoption step (inventory write-up, the questions, the as-built reconstruction, the conformance sweep, the gap audit), each with its hours, plus `docs/sprints/deferred.md` and a generated `docs/.keel/plan.json`. Show it to the user in the same message. Generate `scripts/keel-time`, create `docs/sessions.md`, gitignore `docs/.keel/clock.jsonl`, and run `scripts/keel-time start` before the first adoption slice, so the adoption itself is timed (SKILL.md, "Session time — measured, never remembered"). Each step's commit marks its slice `done` with its measured `actual_hours`. At step 5, the "fix now" bucket becomes the next sprint and "fix when touched" goes to `deferred.md`, with hours, so the question "how long until this project is where Keel wants it?" has a number from the day it was adopted. Only the user's explicit statement that they do not want sprints changes this — recorded as `Sprints: off` with its D-entry.

...
| 5. Development | Sprint planning, vertical slices with test points and evidence (commands, outputs, commit hashes), unit/integration/e2e coverage per acceptance criterion, real playground with synthetic seed data, independent review subagents, affected-test sprint closes, lessons learned |
...
**The work never lives only on your computer.** Before Keel creates a single file it checks, with commands rather than assumptions, that the project will survive the machine it is being built on: a Git repository with a remote, or a tree that replicates off the machine (Dropbox, iCloud Drive, Google Drive, OneDrive and equivalents — confirmed with the user, never guessed from a folder name). A repository with no remote is a partial answer, not a pass. Whatever is missing comes back as a concrete offer — `git init` and the first commit, publishing to the forge you name, or moving the project into a synced folder — and declining is your right, recorded as an accepted risk with its consequence written down instead of silently assumed. From there on nothing is left uncommitted: every working block ends with the work committed to `develop` or to a branch bound for it (a repo with only `main`/`master` gets `develop` created first), and `scripts/keel-verify` fails a dirty tree or a remoteless repo that carries no such decision.

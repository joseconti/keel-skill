...
This guide explains how to install the Keel skill in your Claude environment. Keel is a [Claude skill](https://docs.claude.com): a directory containing a `SKILL.md` file plus its supporting references. Once placed in a location where your Claude client looks for skills, it is invoked automatically when the user's request matches its trigger description.
...
Or download a release archive from the GitHub releases page and extract it. The release archives are produced by `git archive` and already exclude repository-only files via `.gitattributes` `export-ignore`, so the archive's root will be the `keel/` directory contents.

## Install location

Where to copy `keel/` depends on how you use Claude.

### Option A — Personal skill (single user)

Most Claude clients support a user-level skills directory. Copy the `keel/` folder into that directory.
...
If you are not sure where your client looks for user skills, check its documentation under "skills", "custom skills", or "user skills". The destination must contain `keel/SKILL.md` directly (not `keel/keel/SKILL.md`).
...
For a team installation, host the skill in a shared location your team's Claude environments already read from (an internal plugin marketplace, a shared skills folder synced via your existing tooling, etc.). The skill is plain files; any mechanism that delivers a directory will work, as long as `SKILL.md` ends up at the root of the installed folder.
...
- Confirm your Claude client's skills directory is correctly configured.
